...
export const Input: React.FC<Props> = (props) => {
  ...
  const renderInput = () => {
    switch (type) {
      case 'text':
        return <input d-input-text type={type} placeholder={placeholder}
      ...
      case 'sms_code':
        return (
          <div flex gap-x-16px>
            <input shrink-1 d-input-text type="text" placeholder={placeholder} max-w="[calc(40%-8px)]"
              value={value} onChange={e => onChange?.(e.target.value)} />
            <button max-w="[calc(60%-8px)]" shrink-0 d-btn>发送验证码</button>
          </div>
        )
      default:
        return null
